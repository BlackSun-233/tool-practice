...
a.txt  b.txt
```
glob是Shell在执行命令之前做的文件名模式展开，不是ls程序本身实现。
* 匹配任意长度任意字符；? 严格匹配单个字符；{a,b,c}批量展开多个文件名。
...
如果cd是一个外部独立程序，运行时会开启子进程；子进程修改自己的目录，不会影响父Shell，命令结束后就失效。
因此cd必须作为Shell内置命令，直接在当前Shell进程内部修改工作目录。
